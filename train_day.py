...
LAG_SEQUENCE = [273, 274, 275, 276, 277, 278, 279, 280, 281, 282, 283, 284, 285, 286, 287, 288, 289]
NUM_STATIC_REAL_FEATURES = 224
NUM_DYNAMIC_REAL_FEATURES = 27
PARTIAL = 100
...
def evaluate(model, model_path, val_dataloader, val_dataset, device, config):
    ...
    for item_id, ts in enumerate(val_dataset):
        ...
        rmsle.append(np.sqrt(sum(ground_truth - forecast_median[item_id])**2 / len(ground_truth)))
    return {
        'mean_value': round((np.expm1(forecast_median[:, :4])).mean()),
        'rmsle': round(sum(rmsle) / NUM_INN_ID, 4),
    }
...
def get_dataloader(train_dataset, val_dataset, predict_dataset, config):
    train_dataloader = create_train_dataloader(
        ...
        freq=FREQ,
        data=train_dataset,
        batch_size=3118,  #51963
        num_batches_per_epoch=901, #901
    )
...
def train(config, train_dataloader, val_dataloader, predict_dataloader, val_dataset, add_val=True):
    ...
    device = 'cuda:0'  #'cpu'
    optimizer = AdamW(model.parameters(), lr=6e-4, betas=(0.9, 0.95))  #lr=6e-4,, weight_decay=1e-1
    ...
    for epoch in range(2000):
        ...
        if add_val:
            ...
            accelerator.save_state(
                foldername / f'checkpoint-epoch={epoch+1}-train_loss={loss_mean}-rmsle={score['rmsle']}'
            )
            if score_min is None:
                score_min = score['rmsle']
            elif score_min < score['rmsle']:
                patience -= 1
            else:
                score_min = score['rmsle']
                patience = PARTIAL
...
def run_pipeline(add_val):
    ...
    if not (add_val is None):
        ...
        train_dataset, val_dataset, predict_dataset = load_dataset(DATASET_DAY_PATH, add_val)
        logger.info(
